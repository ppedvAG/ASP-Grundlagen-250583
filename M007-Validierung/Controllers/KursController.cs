...
public class KursController(KursDBContext db) : Controller
{
	...
	public IActionResult InhaltSpeichern(int id, string inhalt)
	{
		...
		db.SaveChanges();
		return RedirectToAction("KursDetails", new { id });
	}

	public IActionResult KursnameBearbeiten(int id)
	{
		Kurse k = db.Kurse.Single(e => e.Id == id);
		return View(k);
	}

	public IActionResult NameAnpassen([Bind] Kurse k)
	{
		//ModelState.IsValid: Prüft, ob die Validierung Clientseitig funktioniert hat
		//Sollte bei Validierung immer verwendet werden
		if (!ModelState.IsValid)
		{
			return BadRequest();
		}

		//Änderungen vom Frontend in die Datenbank übernehmen
		db.Kurse.Update(k);
		db.SaveChanges();

		return RedirectToAction("AlleKurse");
	}
}
